...
test('kênh Ask đi trọn vòng, và ask_wait chặn thật', async ({ stack }, testInfo) => {
  ...
  for (let i = 0; i < QUESTIONS.length; i++) {
    ...
    const item = await waitAnswered(worker, THREAD_KEY, id!);
    const now = Date.now();
  ...
  const last = (await readThread(worker, THREAD_KEY)).find((i) => i.id === ids[2]);
  ...
  const rendered = await pierceHtml(page, 'lmx-a');
